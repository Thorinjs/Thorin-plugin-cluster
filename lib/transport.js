  ...
  class ClusterTransport extends thorin.Interface.Transport {
    ...
    handleConnection(socket) {
      ...
      socket.on('message', (msg) => {
        ...
        if (msg.e === EVENT.PING) { // heartbeat event
          return this.sendEvent(socket, EVENT.PONG);
        }
    ...
    sendEvent(socket, event, data, fn) {
      if (!socket.__authenticated) return;
      let d = {
        e: event
      };
      ...
      try {
        ...
        logger.warn(`Could not stringify intent result in action ${d.type || ''}`);
        console.log(d);
        console.trace(e);
        return;
      }
      try {
        ...
        logger.warn(`Could not send intent in action ${d.type || ''}`);
        console.trace(e);
      }
    }
