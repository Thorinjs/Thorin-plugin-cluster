...
module.exports = (thorin, opt, pluginObj) => {
  ...
  class ClusterTransport extends thorin.Interface.Transport {
    ...
    handleEvent(socket, event, data, fnId) {
      switch (event) {
        case EVENT.ACTION_CALL:
          if (typeof data === 'object' && data && typeof data.type === 'string') {
            return this.handleRequest(socket, data.type, data.payload || {}, fnId);
          }
          break;
        case EVENT.ABORT:
    ...
    handleConnection(socket) {
      ...
      socket.on('message', (msg) => {
        ...
        return this.handleEvent(socket, msg.e, msg.d, msg.fn);
      });
      socket.on('close', () => {
    ...
    handleActionDispatch(actionObj, socket, payload, fnId) {
    ...
    handleRequest(socket, actionType, payload, fnId) {
      ...
      this[providers][actionType](socket, payload, fnId);
    }
